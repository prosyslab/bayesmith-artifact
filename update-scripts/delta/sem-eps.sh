...
if [[ "$REUSE" = "reuse" ]]; then
  ...
  bingo/mb-eps.py $NEW_OUTPUT_DIR/$ANALYSIS/bnet/trans_named_cons_all.txt.fx \
...
sed 's/(/NEW(/' $NEW_OUTPUT_DIR/$ANALYSIS/bnet/Alarm.txt > $NEW_OUTPUT_DIR/$ANALYSIS/$MERGED/SemAlarm.txt
sed 's/(/NEW(/' $NEW_OUTPUT_DIR/$ANALYSIS/bnet/GroundTruth.txt > $NEW_OUTPUT_DIR/$ANALYSIS/$MERGED/SemGroundTruth.txt
...
if [[ "$REUSE" = "reuse" ]]; then
  ...
  bingo/build-bnet.sh $NEW_OUTPUT_DIR/$ANALYSIS rule-prob.txt $NEW_OUTPUT_DIR/$ANALYSIS/$MERGED/SemAllAlarm.txt $MERGED $EPS
fi
...
if [[ "$@" =~ "--bayesmith" ]]; then
  PREFIX=delta_sem-eps_${FB_MODE}_${EPS}_bayesmith_
else
  PREFIX=delta_sem-eps_${FB_MODE}_${EPS}_
fi
...
script/auc.py $NEW_OUTPUT_DIR/$ANALYSIS/bingo_${PREFIX}stats.txt \
              $NEW_OUTPUT_DIR/$ANALYSIS/$MERGED/SemAlarm.txt
