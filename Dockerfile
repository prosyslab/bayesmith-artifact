...
SHELL ["/bin/bash", "-c"]
USER root
RUN cd /tmp && wget https://github.com/souffle-lang/souffle/releases/download/2.0.2/souffle_2.0.2-1_amd64.deb ; \
...
RUN mkdir llvm && cd /tmp && wget https://github.com/TianyiChen/llvm-build/releases/download/48a8c7dc/clang11-virtualroot.tar.gz && tar -xzf clang11-virtualroot.tar.gz -C ~/llvm/
