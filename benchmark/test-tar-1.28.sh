...
rm foo bar
   tar xf foo.tar -X foo.exclude -X bar.exclude
test ! -f foo -a ! -f bar
# FEATURE: CONFIG_FEATURE_TAR_FROM
# FEATURE: CONFIG_FEATURE_TAR_CREATE
mkdir foo
...
echo foo/bar >foobar.exclude
   tar xf foo.tar foo -X foobar.exclude
...
t=`tar2_bz2 | bunzip2 |    tar tvf -`
test x"$res2" = x"$t"
t=`tar2_bz2 | bunzip2 |    tar tv`
test x"$res2" = x"$t"
