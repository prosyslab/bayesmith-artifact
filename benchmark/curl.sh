...
docker run --mount type=bind,source=/dev/shm/$F/,target=/src sparrow:latest bash -c "cd /src;CC=clang;./configure;"
docker run --mount type=bind,source=/dev/shm/$F/,target=/src sparrow:latest bash -c 'cd /src;find . -name "*.c"|xargs -I{} -P 8 clang-tidy {}  -fix-errors -checks="-*,readability-braces-around-statements" -- $INC -Iinclude 2>&1 #|grep error:'
