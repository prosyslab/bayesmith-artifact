...
#    plt.plot([-0.3, 9.3], [1, 1], linestyle='--', color='royalblue', zorder=1)
    ...
    for name, path in programs:
        ...
        i = 0
        epsilons = [str(x) for x in [0.001, 0.01, 0.1]] #, 0.1 ]
        for eps in epsilons:
            yidx = 0
            trues = []
            iteration_file = path + '/bingo_delta_sem-eps_strong_' + eps + '_stats.txt'
            ...
            if xidx == 0:
                plt.bar(xidx + (i-1) * (bar_width + 0.02), iters / base_iters, bar_width, color=color[eps],
...
#    plt.plot([-0.3, 9.3], [1, 1], linestyle='--', color='royalblue', zorder=1)
    ...
    bar_width=0.27
    for name, path in programs:
        base_file = path + '/bingo_delta_sem-eps_strong_0.001_combined/init.out'
        try:
            base_rank = int([l for l in open(base_file) if 'TrueGround' in l][-1].split('\t')[0])
        except:
            base_rank = 0
        i = 0
        for eps in epsilons:
            yidx = 0
            trues = []
            iteration_file = path + '/bingo_delta_sem-eps_strong_' + eps + '_combined/init.out'
            try:
                rank = int([l for l in open(iteration_file) if 'TrueGround' in l][-1].split('\t')[0])
            except:
                rank = 0
            if xidx == 0:
                plt.bar(xidx + (i-1) * (bar_width + 0.02), rank / base_rank, bar_width, color=color[eps],
                        hatch=hatch[eps], edgecolor=ecolor[eps], zorder=2,
                        label=(r'$\epsilon =$' + eps))
            else:
                plt.bar(xidx + (i-1) * (bar_width + 0.02), rank / base_rank, bar_width, color=color[eps],
            ...
            if rank / base_rank > 2:
                if xidx == 5:
                    ax.annotate('{0:.1f}'.format(rank/base_rank), xy = (xidx + (i - 1.7) * (bar_width + 0.1), 1.01), size=8, xycoords=trans)
                elif xidx == 8 and eps == '0.005':
                    ax.annotate('{0:.1f}'.format(rank/base_rank), xy = (xidx + (i - 1.6) * (bar_width + 0.1), 1.01), size=8, xycoords=trans)
                else:
                    ax.annotate('{0:.1f}'.format(rank/base_rank), xy = (xidx + (i - 1.5) * (bar_width + 0.1), 1.01), size=8, xycoords=trans)
            i += 1
        xidx += 1
    plt.legend(fontsize=tickfontsize)
    plt.tight_layout()
    plt.savefig("sensitivity-init.pdf")
